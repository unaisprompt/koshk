                ...
                <ul>
                    ...
                    @else
                        @if ($data->category)
                            ...
                            @if ($data->subcategory)
                                <li> <a href="{{ url('products?category_id=' . $data->category->id . '&subcategory_id=' . $data->subcategory->id) }}"
                                        title="">{{ $data->subcategory->subcategory_name }}</a><input type="hidden"
                                        id="subcategory_id" value="{{ $data->subcategory->id }}">
                                    @if ($data->inner_subcategory)
                                        <span>/</span>
                                    @endif
                                </li>
                                @if ($data->inner_subcategory)
                                    <li> <strong>{{ $data->inner_subcategory->innersubcategory_name }}</strong><input
                                            type="hidden" id="inner_subcategory_id"
                                            value="{{ $data->inner_subcategory->id }}"> </li>
                                @endif
                            @endif
                        @endif
                    @endif
                        ...
                        <div class="slider-items-products">
                            @if ($data->category)
                                <div id="category-desc-slider" class="product-flexslider hidden-buttons">
                                    <div
                                        class="slider-items slider-width-col1
                        ...
                        <div class="pro-listing">
                                ...
                                <div class="row">
                                    <div class="col-xl-6 col-lg-6 col-md-6 col-3">
                                        <div class="product-list-grid">
                                            <ul class="nav">
                                                <li class="nav-item">
                                                    <a href="{{ url('products-grid') }}?{{ request()->getQueryString() }}"
                                                        class="button-grid
                                        nav-link">
                                                ...
                                                <li class="nav-item">
                                                    <a href="{{ url('products') }}?{{ request()->getQueryString() }}"
                                                        class="button-list
                                        nav-link active">
                            ...
                            <div class="category-products">
                                <ol class="products-list" id="products-list">
                                    ...
                                    @foreach ($data->products as $product)
                                        <li
                                            ...
                                            <div class="product-image" style="display:block;width:100%"> <a
                                                    href="{{ url('product-detail?id=' . $product->id) }}"
                                                    title="{{ $product->product_name }}"> <img class="small-image"
                                            ...
                                            <div class="product-shop" style="display:block;float:left">
                                                ...
                                                <div class="ratings">
                                                    <div class="rating-box">
                                                        <div style="width:{{ $product->rattings ? $product->rattings[0]->avg_ratting * 2 * 10 : '0' }}%"
                                                            class="rating"></div>
                                                    </div>
                                                    <p class="rating-links"> <a href="#">1 Review(s)</a> <span
                                                            class="separator">|</span> <a href="#review-form">Add Your
                                                            Review</a> </p>
                                                </div>
                                                <div class="desc std">
                                                    <p>{{ $product->description }} <a class="link-learn" title=""
                                                            href="{{ url('product-detail?id=' . $product->id) }}">Learn
                                                            More</a> </p>
                                                </div>
                                                <div class="price-box">
                                                    <p class="old-price"> <span class="price-label"></span> <span
                                                            class="price"> AED {{ $product->product_price }} </span> </p>
                                                    <p class="special-price"> <span class="price-label"></span> <span
                                                            class="price"> AED {{ $product->discounted_price }} </span>
                                                    </p>
                                                </div>
                                                ...
                                                <div class="actions">
                                                    <button class="button btn-cart ajx-cart" title="Add to Cart"
                                                        type="button" data-details="{{ json_encode($product) }}"
                                                        onClick="addCart($(this).data('details'),$(this).closest('.item').find('.qty').val())"><span>Add
                                                            to Cart</span></button>
                                                    <span class="add-to-links"> <a title="Add to Wishlist"
                                                            class="button link-wishlist @if ($product->is_wishlist) active @endif"
                                                            href="#"
                                ...
                                @if ($data->total_count > 6)
                                    <a href="#" id="load_more" onclick="event.preventDefault();loadMore()">Load
                                        more...</a>
                                @endif
                            </div>
                            <!-- <div class="pages">
                                    ...
                                    <div class="card border-0 mb-2">
                                        <div class="card-header">
                                            <h6 class="mb-0">
                                                ...
                                                @else
                                                    <a class="link-title"
                                                        href="{{ url('products?category_id=' . $data->category->id . '&subcategory_id=' . $subcategory->id) }}">
                                                        {{ $subcategory->subcategory_name }}
                                                    </a>
                                                @endif
                                            </h6>
                                                ...
                                                <div class="card-body text-muted">
                                                    <ul class="list-unstyled">
                                                        @foreach ($subcategory->inner_category as $innercategory)
                                                            <li> <a
                                                                    href="{{ url('products?category_id=' . $data->category->id . '&subcategory_id=' . $subcategory->id . '&innersubcategory_id=' . $innercategory->id) }}">{{ $innercategory->innersubcategory_name }}</a>
                                                            </li>
                                                        @endforeach
                                                    </ul>
                                                </div>
                            ...
                            @foreach ($data->brands as $brand)
                                <div class="custom-control custom-checkbox mb-2">
                                    <input type="checkbox" value="{{ $brand->id }}"
                                        class="custom-control-input brands" id="brandCheck{{ $loop->iteration }}"
                                        onChange="brandChecked($(this))">
                                    <label class="custom-control-label" for="brandCheck{{ $loop->iteration }}">
                            ...
                            <div class="wrapper">
                                <fieldset class="filter-price">
                                    ...
                                    <div class="price-field">
                                        <input type="range" min="0" max="{{ $data->max_price }}"
                                            value="1" id="lower" onchange="filter()">
                                        <input type="range" min="0" max="{{ $data->max_price }}"
                                            value="{{ $data->max_price }}" id="upper" onchange="filter()">
                                    </div>
                                            ...
                                            <div class="item-inner">
                                                <div class="item-img">
                                                    <div class="item-img-info"> <a class="product-image"
                                                            ...
                                                            href="{{ url('product-detail?id=' . $item->id) }}"> <img
                                                                alt="{{ $item->product_name }}"
                                                                src="{{ $item->productimage->image_url }}"> </a>
                                                        @if ($item->new_item == 1)
                                                            <div class="new-label new-top-left">new</div>
                                                        @endif
                                                        <div class="box-hover">
                                                ...
                                                <div class="item-info">
                                                    <div class="info-inner">
                                                        ...
                                                        <div class="star-rating">
                                                            <span
                                                                style="width:{{ count($item->rattings) > 0 ? $item->rattings[0]->avg_ratting * 2 * 10 : '0' }}%">Rated
                                                                <strong
                                                                    class="rating">{{ count($item->rattings) > 0 ? $item->rattings[0]->avg_ratting : '0' }}</strong>
                                                                out of 5</span>
                                                        </div>
                                                        <div class="item-content">
                                                            <div class="item-price">
                                                                <div class="price-box"> <span class="regular-price"> <span
                                                                            class="price">AED
                                                                            {{ $item->discounted_price }}</span> </span>
                                                                </div>
                                                            </div>
                                                            <div class="action">
            ...
            $.ajax({
                ...
                success: function(response) {
                    response.products.forEach(function(product, index) {
                        var badge = '';
                        if (product.is_new) {
                            badge = `<div class="new-label new-top-left">New</div>`;
                        } else {
                            var percentage = ((product.product_price - product.discounted_price) /
                                product.product_price) * 100;

                            badge =
                                `<div class="sale-label sale-top-left"> ${percentage.toFixed(2)}%</div>`;
                        }
                        var ratting = 0;
                        if (product.rattings && product.rattings.length > 0) {
                            ratting = product.rattings[0].avg_ratting;
                        }
                        var item = `<li  class="item   ${index%2==0?`even`:`odd`} listing">
                                            <div class="product-image" style="display:block;width:100%"> 
                                                <a href="{{ url('product-detail') }}?id=${product.id}" title="${product.product_name}"> 
                                                <img class="small-image" src="${product.productimage.image_url}" alt="${product.product_name}"> </a>
                                                ${badge}
                                            </div>
                                            <div class="product-shop" style="display:block;float:left">
                                                ...
                                                <div class="actions">
                                                    <button class="button btn-cart ajx-cart" title="Add to Cart"
                                                        type="button" data-details="${ product}"
                                                        onClick="addCart($(this).data('details'),$(this).closest('.item').find('.qty').val())"><span>Add
                                                            to Cart</span></button>
                                                    <span class="add-to-links"> <a title="Add to Wishlist"
                                                            class="button link-wishlist ${product.is_wishlist?'active':''}"
                                                            href="#"
                                                            onclick="event.preventDefault();addWishlist(${product.id},$(this))"></a>
                                                        <!-- <a title="Add to Compare" class="button link-compare" href="compare.html"></a> -->
                                                    </span>
                    ...
                    $('#result_count').html(
                        `Showing ${$('.listing').length} of ${response.total_count} results`);
                    qtyUpdate();
                }
            })
        }
